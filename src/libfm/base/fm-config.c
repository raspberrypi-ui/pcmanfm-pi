...
#define _save_config_bool(_str_,_cfg_,_name_) \
    g_key_file_set_integer (kf,_str_,#_name_,_cfg_->_name_);

#define _save_config_int(_str_,_cfg_,_name_) \
    g_key_file_set_integer (kf,_str_,#_name_,_cfg_->_name_);

#define _save_config_string(_str_,_cfg_,_name_) \
    if (_cfg_->_name_ != NULL) g_key_file_set_string (kf,_str_,#_name_,_cfg_->_name_);
...
#define _save_drop_action(_str_,_cfg_,_name_) do { \
    ...
    case FM_DND_DEST_DROP_AUTO: \
        g_key_file_set_string (kf, _str_, #_name_, "auto"); \
        break; \
    case FM_DND_DEST_DROP_COPY: \
        g_key_file_set_string (kf, _str_, #_name_, "copy"); \
        break; \
    case FM_DND_DEST_DROP_MOVE: \
        g_key_file_set_string (kf, _str_, #_name_, "move"); \
        break; \
    case FM_DND_DEST_DROP_ASK: \
        g_key_file_set_string (kf, _str_, #_name_, "ask"); \
        break; \
    } \
} while(0)
...
void fm_config_save(FmConfig* cfg, const char* name)
{
    ...
    if(g_mkdir_with_parents(dir_path, 0700) != -1)
    {
        ...
        kf = g_key_file_new ();
        g_key_file_load_from_file (kf, name, 0, NULL);
        _save_config_bool("config", cfg, single_click);
        _save_config_bool("config", cfg, middle_click);
        _save_config_bool("config", cfg, use_trash);
        _save_config_bool("config", cfg, confirm_del);
        _save_config_bool("config", cfg, confirm_trash);
        _save_config_bool("config", cfg, advanced_mode);
        _save_config_bool("config", cfg, si_unit);
        _save_config_bool("config", cfg, force_startup_notify);
        _save_config_bool("config", cfg, date_iso_8601);
        _save_config_bool("config", cfg, backup_as_hidden);
        _save_config_bool("config", cfg, no_usb_trash);
        _save_config_bool("config", cfg, no_child_non_expandable);
        _save_config_bool("config", cfg, show_full_names);
        _save_config_bool("config", cfg, only_user_templates);
        _save_config_bool("config", cfg, template_run_app);
        _save_config_bool("config", cfg, template_type_once);
        _save_config_int("config", cfg, auto_selection_delay);
        _save_drop_action("config", cfg, drop_default_action);
        _save_config_bool("config", cfg, defer_content_test);
        _save_config_bool("config", cfg, quick_exec);
        _save_config_string("config", cfg, terminal);
        _save_config_string("config", cfg, archiver);
        _save_config_string("config", cfg, format_cmd);
        _save_config_bool("config", cfg, thumbnail_local);
        _save_config_int("config", cfg, thumbnail_max);
        if (cfg->modules_blacklist && cfg->modules_blacklist[0])
        {
            str = g_strjoinv (";", cfg->modules_blacklist);
            g_key_file_set_string (kf, "config", "modules_blacklist", str);
            g_free (str);
        }
        if (cfg->modules_whitelist && cfg->modules_whitelist[0])
        {
            str = g_strjoinv (";", cfg->modules_whitelist);
            g_key_file_set_string (kf, "config", "modules_whitelist", str);
            g_free (str);
        }
        ...
        _save_config_int("ui", cfg, small_icon_size);
        _save_config_int("ui", cfg, pane_icon_size);
        _save_config_int("ui", cfg, thumbnail_size);
        _save_config_bool("ui", cfg, show_thumbnail);
        _save_config_bool("ui", cfg, shadow_hidden);
        if (cfg->list_view_size_units && cfg->list_view_size_units[0])
            cfg->list_view_size_units[1] = '\0'; /* leave only 1 char */
        _save_config_string("ui", cfg, list_view_size_units);
        _save_config_string("ui", cfg, saved_search);
        _save_config_bool("places", cfg, places_home);
        _save_config_bool("places", cfg, places_desktop);
        _save_config_bool("places", cfg, places_root);
        _save_config_bool("places", cfg, places_computer);
        _save_config_bool("places", cfg, places_trash);
        _save_config_bool("places", cfg, places_applications);
